$.fn.toufiqSPA = function (options) {
    ...
    window.history && window.history.pushState && $(window).on("popstate", function () {
        ...
        null == strEval && $ajaxTarget.addClass("spa-loader"), "post" == method.toLowerCase() ? $.ajax({
            type: method,
            url: url,
            data: formData,
            processData: !1,
            ...
            success: function (data, textStatus, JqrXHR) {
                $ajaxTarget.removeClass("spa-loader"), null != strEval ? eval(strEval) : ($ajaxTarget == $mainSelector && (url = JqrXHR.getResponseHeader("Location"), url != window.location.href.toString().split(window.location.host)[1] && window.history.pushState(null, null, url)), $ajaxTarget.html(data))
            },
            error: function (a, t, r) {
        ...
        null != $dataTarget && ($dataTarget = $($dataTarget), $dataTarget.modal("show"));
        var strEval = $this.attr("eval");
        null == strEval && $ajaxTarget.addClass("spa-loader"), $.ajax({
            type: "get",
            url: url,
            success: function (data, textStatus, JqrXHR) {
                $ajaxTarget.removeClass("spa-loader"), null != strEval ? eval(strEval) : ($ajaxTarget == $mainSelector && (url = JqrXHR.getResponseHeader("Location"), url != window.location.href.toString().split(window.location.host)[1] && window.history.pushState(null, null, url)), $ajaxTarget.html(data))
